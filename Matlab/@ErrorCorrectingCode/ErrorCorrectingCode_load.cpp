...
{
  if (nrhs != inputCount) {
    throw std::invalid_argument("Wrong argin count in ErrorCorrectingCode_load");
  }
  if (nlhs != outputCount) {
    throw std::invalid_argument("Wrong argout count in ErrorCorrectingCode_load");
  }
  
  std::unique_ptr<ErrorCorrectingCode> code;
  try {
    code = load<ErrorCorrectingCode>(prhs[0]);
